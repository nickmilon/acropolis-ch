 ...
 * @example flagsCH.flagsToNum(['resolve']) => an integer that can be used in request options
 * available flags:
...
export class CHclient {
  ...
  /**
   * ❗️private method called by {@link request}, {@link get} and {@link post}.
   * it is not supposed to be called directly we only document it here for caller's reference
   * @async
  ...
  async _request(method = 'GET', query = '', reqBody = null, { chOpts = {}, flags = this._defaultFlags } = {}) { // chOpts
    ...
    try {
      ...
      if (statusCode === 200) { //  below we do flag validation directly for efficiency never change flagsCH order for resolve or event
   ...
   * @param {undefined|null|string} sqlQuery query string
